...
export const ApplicationsContextProvider = ({ children }) => {
  const [applicationsState, dispatch] = useReducer(applicationsReducer, initialState)
  ...
  const value = useMemo(() => ({ applicationsState, applyForJob, setUserJobsAppliedFor }), [
    applicationsState,
    applyForJob,
    setUserJobsAppliedFor,
  ])
...
export const selectJobIdsAppliedFor = (applicationsState) => Object.keys(applicationsState.data).map(Number)
